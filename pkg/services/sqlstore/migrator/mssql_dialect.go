...
// Source: https://github.com/grafana/grafana/pull/12204

// A partir daqui, juntei funçoes que o grafana estava a dizer que estavam em falta.

func (db *Mssql) IndexCheckSql(tableName, indexName string) (string, []interface{}) {
	args := []interface{}{}
	// Versao em MySQL:
	// sql := "SELECT 1 FROM " + db.Quote("INFORMATION_SCHEMA") + "." + db.Quote("STATISTICS") + " WHERE " + db.Quote("TABLE_SCHEMA") + " = DB_NAME() AND " + db.Quote("TABLE_NAME") + "=? AND " + db.Quote("INDEX_NAME") + "=?"

	// TODO: Tentativa de versao em SQLServer:
	println(string("Funçao nao implementada - IndexCheckSql."))
	println(string("aqui vai o sql: "))
	// problema ta nas aspas/parentesis retos?
	sql := "SELECT 1 FROM sys.indexes WHERE name='" + indexName + "' AND object_id(N'" + tableName + "')"
...
func (db *Mssql) IsUniqueConstraintViolation(err error) bool {
	/*if driverErr, ok := err.(*mssql.MSSQLError); ok {
		if driverErr.Number == mssqlerr.ER_DUP_ENTRY {
			return true
		}
	}*/
	// TODO
	fmt.Printf("Foi chamado o metodo IsUniqueConstraintViolation() mas nao está implementado em mssql!!!!!!!")
